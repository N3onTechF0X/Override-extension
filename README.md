...
[^1]: Который есть в базе расширения. В репозитории скрипта [Overrider](https://github.com/N3onTechF0X/Overrider) в разделах `Известные проблемы` и `Скины` будет написано о не найденных скинах, если таковые есть.  
...
Возле каждой пушки/корпуса/дрона есть два выбора: первый это ваш текущий скин, который нужно заменить на другой (default если без скина), второй это какой скин вы хотите получить.  
![Сайт скинов](./images/skins.png)  
  
Например, если нужно заменить рикошет XT на RF, найдите строку с пушкой «ricochet» (зависит от выбранного языка), в первом поле выберите XT, а во втором — RF. Перейдите на вкладку с танками и обновите страницу (не забудьте включить расширение (1)).  
![Пример смены скина](./images/example.png)  
Теперь, если вы оденете рикошет ХТ, вы увидите вместо него скин RF.  

## Другая информация

Дополнительная информация, включая разделы `Скины`, `Известные проблемы`, `ЧаВо`, `Благодарности` и `Обратная связь`, доступна в [ReadMe скрипта Overrider](https://github.com/N3onTechF0X/Overrider/blob/main/README.md).  
